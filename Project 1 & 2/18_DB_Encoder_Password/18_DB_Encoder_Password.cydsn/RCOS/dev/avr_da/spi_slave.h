/** @file       spi_slave.h
 *  @brief      Header file of SPI_SLAVE device
 ...
 *  @ingroup    DEV_COM
 *  @brief      Slave Serial Peripheral Interface for Microchip AVR DA
 *  @{
 */
#define MISO_PIN (consts->mosiPin + 1)
#define SCK_PIN (consts->mosiPin + 2)
#define SS_PIN (consts->mosiPin + 3)
 ...
 */
#define DEV_COM_SPI_SLAVE_CREATE(_name, _base, _mode, _port, _mosiPin, _alt, \
    ...
    tsSpiSlaveParams _name##Params =                                         \
        {                                                                    \
            &_name##RxQ,                                                     \
            &_name##TxQ,                                                     \
    };                                                                       \
    const tsSpiSlaveConsts _name##Consts =                                   \
        {                                                                    \
            &_base,                                                          \
            &PORT##_port,                                                    \
    ...
    DEV_COM_CREATE(_name, devComSpiSlaveFuncs, &_name##Params, &_name##Consts)
