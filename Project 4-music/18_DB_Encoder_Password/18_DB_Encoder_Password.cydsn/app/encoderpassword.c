...
{
    PATTERN_NODE(NOTE_5_MI, 150),
    PATTERN_NODE(0, 50),
    PATTERN_NODE(NOTE_5_MI, 100),
    PATTERN_NODE_END(0)
};

const tsPatternNode press1[] =
{
    PATTERN_NODE(NOTE_7_SOL, 500),
    PATTERN_NODE(0, 50),
    PATTERN_NODE(NOTE_7_SOL, 500),
    PATTERN_NODE_END(0)
};

const tsPatternNode press2[] =
{
    PATTERN_NODE(NOTE_4_FA, 500),
    PATTERN_NODE(0, 50),
    PATTERN_NODE(NOTE_4_FA, 500),
    PATTERN_NODE_END(0)
};
const tsPatternNode press3[] =
{
    PATTERN_NODE(NOTE_6_DO, 500),
    PATTERN_NODE(0, 50),
    PATTERN_NODE(NOTE_6_DO, 500),
    PATTERN_NODE_END(0)
};
const tsPatternNode press4[] =
{
    PATTERN_NODE(NOTE_7_LA, 500),
    PATTERN_NODE(0, 50),
    PATTERN_NODE(NOTE_7_LA, 500),
    PATTERN_NODE_END(0)
};
const tsPatternNode press5[] =
{
    PATTERN_NODE(NOTE_4_SI, 500),
    PATTERN_NODE(0, 50),
    PATTERN_NODE(NOTE_4_SI, 500),
    PATTERN_NODE_END(0)
};
...
static PROCESS_HANDLER_PROTO(encoderPasswordInitialStateHandler)
{
    ...
    switch (eventCurrent.event)
    {
        ...
        case p70Released:
        {
            ...
            devIoPut(consts->sevenSegmentDisplay,readValue);
            if(readValue == 1) {
                devIoPut(consts->sevenSegmentDisplay,readValue);
                devIoPut(consts->buzzer, (uint32_t)press1);
                 }
            else if(readValue == 2) {
                devIoPut(consts->sevenSegmentDisplay,readValue);
                devIoPut(consts->buzzer, (uint32_t)press2);
            }
            else if(readValue == 3) {
                devIoPut(consts->sevenSegmentDisplay,readValue);
                devIoPut(consts->buzzer, (uint32_t)press3);
            }
            else if(readValue == 4) {
                devIoPut(consts->sevenSegmentDisplay,readValue);
                devIoPut(consts->buzzer, (uint32_t)press4);
            }
            else {
                devIoPut(consts->sevenSegmentDisplay,readValue);
                devIoPut(consts->buzzer, (uint32_t)press5);
            }
